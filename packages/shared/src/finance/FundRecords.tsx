...
import {Alert,Box,Button,Chip,Drawer,LinearProgress,MenuItem,Paper,Stack,Table,TableBody,TableCell,TableContainer,TableHead,TableRow,TextField,Typography} from '@mui/material';
...
export default function FundRecords({admin=false,recordId}:{admin?:boolean;recordId?:string}){
 ...
 const selected=recordId||params.get('record')||undefined;
 const base=admin?'/finance/fund-records':'/portal/fund-records';
 const close=()=>{const q=new URLSearchParams(params);q.delete('record');navigate(base+(q.size?'?'+q:''),{replace:true})};
 return <>
 <FundRecordsContent admin={admin}/>
 <Drawer anchor="right" open={Boolean(selected)} onClose={close} PaperProps={{className:admin?'finance-workspace':undefined,role:'dialog','aria-modal':true,'aria-labelledby':'fund-record-title',sx:{width:{xs:'100%',sm:580},maxWidth:'100vw'}}}>
 <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{px:3,py:2,borderBottom:1,borderColor:'divider'}}><Typography id="fund-record-title" variant="h6">资金记录详情</Typography><Button onClick={close} aria-label="关闭资金记录详情">关闭</Button></Stack>
 <Box sx={{p:{xs:2,sm:3},overflowY:'auto',minHeight:0}}>{selected&&<FundRecordsContent key={selected} admin={admin} recordId={selected}/>}</Box>
 </Drawer>
 </>
}
function FundRecordsContent({admin=false,recordId}:{admin?:boolean;recordId?:string}){
 const base=admin?'/finance/fund-records':'/portal/fund-records',api=admin?'/admin-api/v1/fund-records':'/client-api/v1/fund-records';
 const navigate=useNavigate();const [params,setParams]=useSearchParams();const listParams=new URLSearchParams(params);listParams.delete('record');const search=listParams.toString();
 const [draft,setDraft]=useState<Record<string,string>>(()=>Object.fromEntries(listParams)),[data,setData]=useState<FundRecordsResult|null>(null),[error,setError]=useState(''),[busy,setBusy]=useState(true),[reload,setReload]=useState(0);
 ...
 const detail=(r:FundRecord)=>{const q=new URLSearchParams(search);q.set('record',r.id);return base+'?'+q};
 ...
 const row=data?.record,page=Number(params.get('page')||0),hasFilters=[...listParams.keys()].some(k=>k!=='page');
 ...
 <Stack direction="row" justifyContent="space-between" alignItems="center" gap={1}>{!recordId&&<Typography variant="h5">资金记录</Typography>}<Button disabled={busy} onClick={()=>setReload(x=>x+1)}>刷新</Button></Stack>
 {!recordId&&<Paper variant="outlined" component="form" onSubmit={e=>{e.preventDefault();apply()}} sx={{p:{xs:2,sm:2.5}}}>
 ...
 {admin?<Box sx={{gridColumn:'1 / -1'}}><details className="finance-records-advanced" key={search} open={Boolean(draft.from||draft.to||draft.cardId||draft.customerId)}><summary>日期与关联筛选{draft.from||draft.to||draft.cardId||draft.customerId?' · 已设置':''}</summary><Box sx={{display:'grid',gridTemplateColumns:{xs:'1fr',sm:'repeat(2,minmax(0,1fr))',lg:'repeat(4,minmax(0,1fr))'},gap:2,mt:2}}>
 ...
 {admin&&<TextField size="small" label="客户 ID" value={draft.customerId||''} onChange={e=>update('customerId',e.target.value)} helperText="仅查询有权查看的客户"/>} </Box></details></Box>:<> {['from','to'].map(k=><TextField key={k} size="small" type="date" label={k==='from'?'开始日期（UTC，含）':'结束日期（UTC，不含）'} InputLabelProps={{shrink:true}} value={draft[k]?.slice(0,10)||''} onChange={e=>update(k,e.target.value?e.target.value+'T00:00:00Z':'')}/>)}
 <TextField size="small" label="关联卡片 ID" value={draft.cardId||''} onChange={e=>update('cardId',e.target.value)} helperText="卡片后四位可在关键词中搜索"/>
 ...
 </Box>{row.cardId&&<Button component={Link} to={base+'?'+new URLSearchParams({cardId:row.cardId,...(admin?{customerId:row.customerId}:{})})} sx={{alignSelf:'flex-start'}}>查看此卡资金记录</Button>}{row.originalId&&<Button component={Link} to={detail({...row,id:row.originalId})} sx={{alignSelf:'flex-start'}}>查看原资金记录</Button>}
