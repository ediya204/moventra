...
type Event={id:string;amount:string;txHash:string;providerStatus:string;receivedAt:string};
type Snapshot={address:{network:string;address:string;state:string};events:Event[];postingEnabled:false;mode:'observation'};
export default function DepositAddress({customerId,basePath}:{customerId:string;basePath:string}){
 ...
 <Paper variant="outlined" sx={{p:{xs:2,md:3}}}><Stack spacing={3}><Typography variant="h5">USDT 充值</Typography><Alert severity="info">当前提供地址及渠道记录查询。到账核验与自动入账尚未启用，请勿转入资金。</Alert>
 ...
 <Paper variant="outlined" sx={{p:{xs:2,md:3}}}><Stack spacing={2}><Typography variant="h6">{event?'充值通知详情':'最近充值通知'}</Typography><Stack direction="row" spacing={1}>{event?<Button component={Link} to={basePath+'/deposit'}>返回充值</Button>:<><Button component={Link} to={basePath+'/deposit?page=0'}>查看全部</Button><Button disabled={page===0} component={Link} to={basePath+'/deposit?page='+(page-1)}>上一页</Button><Button disabled={!data||data.events.length<5} component={Link} to={basePath+'/deposit?page='+(page+1)}>下一页</Button></>}</Stack><Typography color="text.secondary">渠道通知不代表已入账；金额在链上核验完成前不计入可用余额。</Typography>{data?.events.map(ev=><Stack key={ev.id} spacing={1}><Typography>{new Date(ev.receivedAt).toLocaleString()} · TRC20 · {ev.amount} USDT</Typography><Typography>待核验 · 未入账</Typography>{!event&&<Button component={Link} to={basePath+'/deposit?event='+ev.id}>详情</Button>}<Typography sx={{overflowWrap:'anywhere'}}>交易哈希：{ev.txHash||'渠道未提供'}</Typography></Stack>)}{data&&!data.events.length&&<Typography color="text.secondary">暂无充值通知</Typography>}</Stack></Paper>
 </Stack>;
}
