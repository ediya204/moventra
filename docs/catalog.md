# Markdown 完整目录

> 由 `pnpm docs:index` 生成；请勿手改。此清单只表示文件存在，不代表能力、部署或验收状态。

当前能力见[状态摘要](current-state.md)，推荐阅读见[文档索引](README.md)，维护方式见[Harness](harness/README.md)。

## 仓库根目录

- [Moventra 开发约束](../AGENTS.md) — `AGENTS.md`
- [Moventra](../README.md) — `README.md`

## apps/admin

- [Moventra 运营后台](../apps/admin/README.md) — `apps/admin/README.md`

## apps/client

- [Moventra 客户端](../apps/client/README.md) — `apps/client/README.md`

## deploy

- [卡片详情导航发布](../deploy/2026-09-07-card-detail-navigation.md) — `deploy/2026-09-07-card-detail-navigation.md`
- [资金运营概览发布记录](../deploy/2026-09-07-operations-overview.md) — `deploy/2026-09-07-operations-overview.md`
- [余额查询与人工资金代码发布](../deploy/2026-09-18-balance-query-release.md) — `deploy/2026-09-18-balance-query-release.md`
- [BIN catalogue release — 2026-09-18](../deploy/2026-09-18-bin-catalog-sync.md) — `deploy/2026-09-18-bin-catalog-sync.md`
- [卡片事件驱动与状态操作发布](../deploy/2026-09-18-card-controls.md) — `deploy/2026-09-18-card-controls.md`
- [卡片当前状态同步发布](../deploy/2026-09-18-card-state-sync.md) — `deploy/2026-09-18-card-state-sync.md`
- [客户端开卡与八个 BIN 目录发布](../deploy/2026-09-18-client-issuing-release.md) — `deploy/2026-09-18-client-issuing-release.md`
- [已完成改动统一发布清单](../deploy/2026-09-18-completed-work-release.md) — `deploy/2026-09-18-completed-work-release.md`
- [TRC20 限额充值自动入账发布与验收](../deploy/2026-09-18-deposit-pilot.md) — `deploy/2026-09-18-deposit-pilot.md`
- [资金中心四流程发布](../deploy/2026-09-18-funds-center-release.md) — `deploy/2026-09-18-funds-center-release.md`
- [客户端开卡结构生产迁移 — 2026-09-18](../deploy/2026-09-18-issuing-checkout-migration.md) — `deploy/2026-09-18-issuing-checkout-migration.md`
- [开卡生产连接准备（已部署，只读）](../deploy/2026-09-18-issuing-preparation.md) — `deploy/2026-09-18-issuing-preparation.md`
- [正式充值与 OTC 分能力接入（2026-09-18）](../deploy/2026-09-18-production-funds-activation.md) — `deploy/2026-09-18-production-funds-activation.md`
- [生产资金展示发布（2026-09-18）](../deploy/2026-09-18-production-funds-view.md) — `deploy/2026-09-18-production-funds-view.md`
- [后端运行优化发布](../deploy/2026-09-18-runtime-optimization.md) — `deploy/2026-09-18-runtime-optimization.md`
- [Moventra completed-session release — 2026-09-18](../deploy/2026-09-18-session-consolidation.md) — `deploy/2026-09-18-session-consolidation.md`
- [全部本地更新统一发布 — 2026-09-19](../deploy/2026-09-19-all-local-release.md) — `deploy/2026-09-19-all-local-release.md`
- [卡片中心布局与状态发布](../deploy/2026-09-19-card-center-release.md) — `deploy/2026-09-19-card-center-release.md`
- [卡片详情与CVV发布（2026-09-19）](../deploy/2026-09-19-card-detail-cvv-release.md) — `deploy/2026-09-19-card-detail-cvv-release.md`
- [卡片列表精简发布（2026-09-19）](../deploy/2026-09-19-card-list-refinement-release.md) — `deploy/2026-09-19-card-list-refinement-release.md`
- [卡片历史与指标生产发布 — 2026-09-19](../deploy/2026-09-19-card-metrics-release.md) — `deploy/2026-09-19-card-metrics-release.md`
- [2026-09-19 会话改动统一发布](../deploy/2026-09-19-consolidated-ui-release.md) — `deploy/2026-09-19-consolidated-ui-release.md`
- [统一资金记录第二批发布 — 2026-09-19](../deploy/2026-09-19-fund-records-release.md) — `deploy/2026-09-19-fund-records-release.md`
- [2026-09-19 单张开卡试运行交付与发布](../deploy/2026-09-19-issuing-pilot.md) — `deploy/2026-09-19-issuing-pilot.md`
- [正式人工出入金启用与原单入账 — 2026-09-19](../deploy/2026-09-19-manual-funds-activation.md) — `deploy/2026-09-19-manual-funds-activation.md`
- [生产消息与统一钱包迁移、开关调整 — 2026-09-19](../deploy/2026-09-19-production-migrations-and-switches.md) — `deploy/2026-09-19-production-migrations-and-switches.md`
- [会话修复上线与换机交接](../deploy/2026-09-19-session-and-handoff.md) — `deploy/2026-09-19-session-and-handoff.md`
- [2026-09-19 交易页面发布](../deploy/2026-09-19-transaction-ui-release.md) — `deploy/2026-09-19-transaction-ui-release.md`
- [Moventra 部署记录](../deploy/README.md) — `deploy/README.md`

## deploy/blnk

- [Moventra Blnk 本地影子账本](../deploy/blnk/README.md) — `deploy/blnk/README.md`
- [本地 shadow 运行监控与恢复](../deploy/blnk/monitoring-and-recovery.md) — `deploy/blnk/monitoring-and-recovery.md`

## deploy

- [Slash 普通通知试接入（2026-09-18）](../deploy/slash-webhook-online-2026-09-18.md) — `deploy/slash-webhook-online-2026-09-18.md`

## docs

- [Moventra 开发总纲](DEVELOPMENT.md) — `docs/DEVELOPMENT.md`
- [Moventra 文档索引与项目现状](README.md) — `docs/README.md`

## docs/api

- [交易与资金 API 契约草案](api/contract.md) — `docs/api/contract.md`

## docs/audits

- [全站字段与接口审计](audits/site-field-api-audit.md) — `docs/audits/site-field-api-audit.md`

## docs/business

- [全站业务逻辑总览](business/README.md) — `docs/business/README.md`
- [FLOW-ADMIN-FINANCE：正式后台财务入口接通](business/admin-finance-migration.md) — `docs/business/admin-finance-migration.md`
- [FLOW-admin-session-persistence：后台刷新恢复登录](business/admin-session-persistence.md) — `docs/business/admin-session-persistence.md`
- [后端运行优化 FLOW-RUNTIME-001](business/backend-runtime-2026-09-18.md) — `docs/business/backend-runtime-2026-09-18.md`
- [FLOW-BIN-CATALOG-01：Slash 目录同步与正式 BIN 管理](business/bin-catalog-sync-2026-09-18.md) — `docs/business/bin-catalog-sync-2026-09-18.md`
- [FLOW-CARD-DETAIL-02：客户端卡片详情规划](business/card-center-detail-plan-2026-09-18.md) — `docs/business/card-center-detail-plan-2026-09-18.md`
- [docs/business/card-issuing-2026-09-18.md](business/card-issuing-2026-09-18.md) — `docs/business/card-issuing-2026-09-18.md`
- [卡片历史回填与指标（2026-09-19）](business/card-metrics.md) — `docs/business/card-metrics.md`
- [FLOW-CARD-OWNER-01：正式后台读取既有归属](business/card-owner-display.md) — `docs/business/card-owner-display.md`
- [卡片当前状态同步](business/card-state-sync.md) — `docs/business/card-state-sync.md`
- [卡片、渠道与交易业务逻辑](business/cards-and-transactions.md) — `docs/business/cards-and-transactions.md`
- [FLOW-CLIENT-ISSUING-001：客户端开卡闭环](business/client-card-issuing.md) — `docs/business/client-card-issuing.md`
- [连通性整改基线与首批清单](business/connectivity-baseline-2026-09-13.md) — `docs/business/connectivity-baseline-2026-09-13.md`
- [Cregis 隔离充值、提现与 OTC](business/cregis-funds.md) — `docs/business/cregis-funds.md`
- [FLOW-CARD-TEST-01 客户卡片测试快照](business/customer-card-binding.md) — `docs/business/customer-card-binding.md`
- [业务闭环开发与验收标准](business/delivery-standard.md) — `docs/business/delivery-standard.md`
- [客户充值地址标准接入](business/deposit-address-integration.md) — `docs/business/deposit-address-integration.md`
- [已有后台功能恢复记录](business/existing-modules-restoration-2026-09-13.md) — `docs/business/existing-modules-restoration-2026-09-13.md`
- [统一资金记录](business/fund-records.md) — `docs/business/fund-records.md`
- [资金、定价与运营业务说明](business/funds-and-operations.md) — `docs/business/funds-and-operations.md`
- [资金中心四流程与正式接入准备](business/funds-center.md) — `docs/business/funds-center.md`
- [生产全局管理员](business/global-administrator.md) — `docs/business/global-administrator.md`
- [正式身份、客户主体与业务开通](business/identity-and-production.md) — `docs/business/identity-and-production.md`
- [统一消息中心](business/message-center.md) — `docs/business/message-center.md`
- [FLOW-TEST-FUNDS-01 正式登录后的测试资金流程](business/online-test-funds.md) — `docs/business/online-test-funds.md`
- [FLOW-TEST-WALLET-01 线上测试余额](business/online-test-wallet.md) — `docs/business/online-test-wallet.md`
- [FLOW-PLATFORM-ADVANCE-001：余额查询与人工出入金](business/platform-advance.md) — `docs/business/platform-advance.md`
- [FLOW-WALLET-001：项目上游钱包与逐卡归属](business/project-wallet.md) — `docs/business/project-wallet.md`
- [页面、API 与浏览器可达范围](business/routes-and-api.md) — `docs/business/routes-and-api.md`
- [两角色与登录隔离](business/two-role-login.md) — `docs/business/two-role-login.md`

## docs

- [Moventra 当前功能与接入状态](current-state.md) — `docs/current-state.md`

## docs/domain

- [V1 资金账本与对账规划](domain/reconciliation-v1-plan.md) — `docs/domain/reconciliation-v1-plan.md`
- [交易、订单与资金领域规范](domain/transactions-and-funds.md) — `docs/domain/transactions-and-funds.md`

## docs/frontend

- [账户目录与详情整理](frontend/account-directory-ui.md) — `docs/frontend/account-directory-ui.md`
- [管理总后台：结构与本地实施](frontend/admin-console.md) — `docs/frontend/admin-console.md`
- [FLOW-ADMIN-TX-OWNER：后台交易尾号与所属账户](frontend/admin-transaction-ownership.md) — `docs/frontend/admin-transaction-ownership.md`
- [正式后台与本地布局统一](frontend/admin-workspace-parity.md) — `docs/frontend/admin-workspace-parity.md`
- [分析聚合 API 契约（产品验证后选用）](frontend/analytics-api-contract.md) — `docs/frontend/analytics-api-contract.md`
- [卡 BIN 详情状态管理（2026-09-07）](frontend/bin-status-management.md) — `docs/frontend/bin-status-management.md`

## docs/frontend/brand

- [Moventra 品牌素材](frontend/brand/README.md) — `docs/frontend/brand/README.md`

## docs/frontend

- [卡片管理闭环（2026-09-07）](frontend/card-administration.md) — `docs/frontend/card-administration.md`
- [全部卡片：可用余额列](frontend/card-available-balance.md) — `docs/frontend/card-available-balance.md`
- [卡 BIN 产品目录与客户端开卡](frontend/card-bin-management.md) — `docs/frontend/card-bin-management.md`
- [发卡渠道与 BIN 产品关联](frontend/card-channels.md) — `docs/frontend/card-channels.md`
- [卡片详情入口修复](frontend/card-detail-navigation.md) — `docs/frontend/card-detail-navigation.md`
- [卡片管理工作台：实施记录与后续接入边界](frontend/card-management-workspace.md) — `docs/frontend/card-management-workspace.md`
- [卡片与交易 Tab / 列表字段规范](frontend/card-transaction-fields.md) — `docs/frontend/card-transaction-fields.md`
- [客户卡片中心：列表与三级详情](frontend/client-cards.md) — `docs/frontend/client-cards.md`
- [客户卡片敏感信息：远程获取、临时展示](frontend/client-cvv.md) — `docs/frontend/client-cvv.md`
- [客户资金中心：USDT 充值、兑换与提现](frontend/client-finance.md) — `docs/frontend/client-finance.md`
- [客户端首期实现](frontend/client-portal.md) — `docs/frontend/client-portal.md`
- [FLOW-CLIENT-TX-DRAWER：客户端交易详情抽屉](frontend/client-transaction-drawer.md) — `docs/frontend/client-transaction-drawer.md`
- [客户端页面布局与导航](frontend/client-workspace-layout.md) — `docs/frontend/client-workspace-layout.md`
- [FLOW-004 客户工作台布局一致性](frontend/client-workspace-parity.md) — `docs/frontend/client-workspace-parity.md`
- [卡交易流水：双币种消费交付与验收](frontend/cross-currency-delivery.md) — `docs/frontend/cross-currency-delivery.md`
- [跨币种卡消费字段差异与实施顺序](frontend/cross-currency-field-gap.md) — `docs/frontend/cross-currency-field-gap.md`
- [数字货币资金工作台（本地隔离实现）](frontend/crypto-finance.md) — `docs/frontend/crypto-finance.md`
- [本地 Demo 快照](frontend/demo-snapshot.md) — `docs/frontend/demo-snapshot.md`
- [资金与财务工作台](frontend/finance-workspace.md) — `docs/frontend/finance-workspace.md`
- [Firebase 登录与 Go 授权](frontend/firebase-setup.md) — `docs/frontend/firebase-setup.md`
- [商户 Logo 展示规范](frontend/merchant-logos.md) — `docs/frontend/merchant-logos.md`
- [客户端消息中心](frontend/message-center.md) — `docs/frontend/message-center.md`
- [FLOW-005 开户审批后默认开放客户端功能](frontend/onboarding-feature-access.md) — `docs/frontend/onboarding-feature-access.md`
- [资金流与运营概览](frontend/operations-overview.md) — `docs/frontend/operations-overview.md`
- [资金对账与盗刷调查模型](frontend/reconciliation-and-fraud-model.md) — `docs/frontend/reconciliation-and-fraud-model.md`
- [收入确认与分析模型](frontend/revenue-model.md) — `docs/frontend/revenue-model.md`
- [账户与卡片风控统计模型](frontend/risk-analytics-model.md) — `docs/frontend/risk-analytics-model.md`
- [Slash Demo 场景、记录ID与预期金额](frontend/slash-demo-scenarios.md) — `docs/frontend/slash-demo-scenarios.md`
- [Slash 清算 Demo：实施与本地验收](frontend/slash-demo.md) — `docs/frontend/slash-demo.md`
- [Slash 字段差异与实施顺序（2026-09-06）](frontend/slash-field-gap.md) — `docs/frontend/slash-field-gap.md`
- [Slash 真实数据接入现有本地后台](frontend/slash-live-data.md) — `docs/frontend/slash-live-data.md`
- [Slash 本地手动同步](frontend/slash-manual-sync.md) — `docs/frontend/slash-manual-sync.md`
- [卡交易流水日期筛选](frontend/transaction-date-range.md) — `docs/frontend/transaction-date-range.md`
- [卡交易详情抽屉与关联卡](frontend/transaction-drawer.md) — `docs/frontend/transaction-drawer.md`
- [卡交易状态分类与视觉规范](frontend/transaction-status-colors.md) — `docs/frontend/transaction-status-colors.md`
- [界面主题与组件约定](frontend/ui-theme.md) — `docs/frontend/ui-theme.md`
- [用户组与开户管理（本地实现）](frontend/user-management.md) — `docs/frontend/user-management.md`
- [V1 费率入口归并](frontend/v1-fee-plans.md) — `docs/frontend/v1-fee-plans.md`
- [V1 个人账户发布范围](frontend/v1-personal-release.md) — `docs/frontend/v1-personal-release.md`
- [官网 SEO 与 Google 收录](frontend/website-seo.md) — `docs/frontend/website-seo.md`

## docs/harness

- [Moventra Harness：工程协作入口](harness/README.md) — `docs/harness/README.md`
- [Markdown 状态维护规则](harness/documentation.md) — `docs/harness/documentation.md`
- [换电脑继续开发](harness/new-computer.md) — `docs/harness/new-computer.md`
- [工程任务与交接模板](harness/task-template.md) — `docs/harness/task-template.md`

## docs/integrations

- [Blnk 用户钱包与多卡分户接入](integrations/blnk.md) — `docs/integrations/blnk.md`
- [Cregis 接入：隔离资金与来源观察](integrations/cregis.md) — `docs/integrations/cregis.md`
- [Slash 白名单问题：处理状态与复用规则](integrations/slash-allowlist.md) — `docs/integrations/slash-allowlist.md`
- [Slash × Moventra × Blnk 集成调查与实施方案](integrations/slash-blnk-plan-2026-09-18.md) — `docs/integrations/slash-blnk-plan-2026-09-18.md`
- [Slash Webhook、API 来源持久化与 Blnk 可靠衔接](integrations/slash-persistence-design-2026-09-18.md) — `docs/integrations/slash-persistence-design-2026-09-18.md`
- [Slash 接入规范](integrations/slash.md) — `docs/integrations/slash.md`

## docs/releases

- [全站业务文档整理记录](releases/2026-09-07-business-docs.md) — `docs/releases/2026-09-07-business-docs.md`
- [全站业务逻辑总览（2026-09-07 历史快照）](releases/2026-09-07-business-overview-snapshot.md) — `docs/releases/2026-09-07-business-overview-snapshot.md`
- [正式页面、Go API 与浏览器可达范围（2026-09-07 历史快照）](releases/2026-09-07-routes-snapshot.md) — `docs/releases/2026-09-07-routes-snapshot.md`
- [V1 功能与接入状态历史快照（2026-09-07）](releases/2026-09-07-state-snapshot.md) — `docs/releases/2026-09-07-state-snapshot.md`
- [V1 本地业务与文档同步验证](releases/2026-09-07-v1-local-sync.md) — `docs/releases/2026-09-07-v1-local-sync.md`
- [官网留言邮件转发（2026-09-07）](releases/2026-09-07-website-contact.md) — `docs/releases/2026-09-07-website-contact.md`
- [官网公司资料与政策发布](releases/2026-09-07-website-legal.md) — `docs/releases/2026-09-07-website-legal.md`
- [首页账户入口移除（2026-09-11）](releases/2026-09-11-remove-home-account-links.md) — `docs/releases/2026-09-11-remove-home-account-links.md`
- [官网移除订阅卡（2026-09-11）](releases/2026-09-11-remove-subscription-cards.md) — `docs/releases/2026-09-11-remove-subscription-cards.md`
- [后台布局与登录隔离联合发布](releases/2026-09-13-admin-login-joint.md) — `docs/releases/2026-09-13-admin-login-joint.md`
- [客户工作台一致性发布](releases/2026-09-13-client-workspace.md) — `docs/releases/2026-09-13-client-workspace.md`
- [已有后台模块恢复与查询发布](releases/2026-09-13-existing-admin-modules.md) — `docs/releases/2026-09-13-existing-admin-modules.md`
- [开户审批与默认功能权限发布](releases/2026-09-13-onboarding-default-features.md) — `docs/releases/2026-09-13-onboarding-default-features.md`
- [正式注册用户目录修复](releases/2026-09-13-registered-user-directory.md) — `docs/releases/2026-09-13-registered-user-directory.md`
- [注册用户账户详情](releases/2026-09-13-user-account-details.md) — `docs/releases/2026-09-13-user-account-details.md`
- [官网 SEO 专项发布](releases/2026-09-17-website-seo.md) — `docs/releases/2026-09-17-website-seo.md`
- [Blnk 私有服务与共享 PostgreSQL 实例（2026-09-18）](releases/2026-09-18-blnk-private-service.md) — `docs/releases/2026-09-18-blnk-private-service.md`
- [Blnk 集成代码发布（2026-09-18）](releases/2026-09-18-blnk-shadow.md) — `docs/releases/2026-09-18-blnk-shadow.md`
- [本地增量同步与部署（2026-09-18）](releases/2026-09-18-local-sync.md) — `docs/releases/2026-09-18-local-sync.md`
- [2026-09-18 main 合并核对](releases/2026-09-18-main-consolidation.md) — `docs/releases/2026-09-18-main-consolidation.md`
- [正式后台卡交易只读投影](releases/channel-projection-2026-09-07.md) — `docs/releases/channel-projection-2026-09-07.md`

## docs/testing

- [Blnk 本地验收记录 — 2026-09-17](testing/blnk-shadow-2026-09-17.md) — `docs/testing/blnk-shadow-2026-09-17.md`
- [交易与资金验收场景](testing/financial-scenarios.md) — `docs/testing/financial-scenarios.md`

## packages/shared

- [Moventra 共享前端模块](../packages/shared/README.md) — `packages/shared/README.md`

## 仓库根目录

- [Moventra 项目说明](../project.md) — `project.md`

## services/api

- [Moventra Go API](../services/api/README.md) — `services/api/README.md`

## services/api/docs

- [账户模型与第一阶段接口边界](../services/api/docs/account-model.md) — `services/api/docs/account-model.md`
- [运营总览只读契约](../services/api/docs/operations-overview.md) — `services/api/docs/operations-overview.md`
- [Go API 验证与历史证据](../services/api/docs/validation.md) — `services/api/docs/validation.md`

## services/local-workspace

- [已恢复的本地业务服务](../services/local-workspace/README.md) — `services/local-workspace/README.md`
