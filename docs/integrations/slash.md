...
| 项目 | 目前结论 | 上线条件/阻塞范围 |
| --- | --- | --- |
| 列表、详情、余额、通知 | SOURCE 有接口；旧本地记录有有限 GET 验证，生产连接及通知未验收 | 租户权限、产品、账户范围和配额验证后才标记连接可用 |
...
## 历史 2026-09-07：独立本地只读预览

当时新增本地 Slash 预览工具（未纳入当前仓库，历史说明见 [只读投影档案](../frontend/slash-live-data.md)），实现单账户有限日期窗口的只读来源快照与本地分页查看。用户提供的只读 Key 经 IP 白名单配置后，真实 `GET /account` 已返回唯一 charge_card 账户。该证据不代表上文统一适配器、Go financial API、Webhook、客户映射或生产对账已实现。余额币种和跨页完整性仍保持未核实；具体运行记录在本地快照数据库保存。
...
统一开卡在持久化快照中固定项目 virtualAccountId，发卡传入该值，并校验返回 account/product/virtualAccount/last4；来源回执仅保存白名单投影字段。依据 [官方创建卡接口](https://docs.slash.com/api-reference/card-post)。本轮使用模拟 Slash 验证，未执行真实发卡；白名单历史问题不作为阻塞。
