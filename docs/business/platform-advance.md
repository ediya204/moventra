...
核对日期：2026-09-18。状态：本地实现及隔离测试通过；未部署、未执行正式入账或真实刷卡。基线为 Moventra main `1ed842adc1abd530dd32f5c123c277d40b41c633` 加本批未提交增量，保留其他任务改动。
...
权限表 `manual_funds_grants` 默认无授权，系统管理员身份不会自动获得资金权限。部署时需单独审阅人员、客户范围和动作权限并受控配置；本批未提供公开授权接口或默认管理员授权。

独立 Worker：在服务目录运行 `go run ./cmd/manual-funds-worker drain` 单次恢复或 `go run ./cmd/manual-funds-worker run` 持续处理。Docker 镜像包含二进制但不会自动启动。复用现有 ledger 环境；disabled 拒绝，shadow 为隔离验证，live 额外要求 `MANUAL_FUNDS_ENABLED=true`。现有 live 对正式账本、认证清单、Cregis 双链及零期初的要求保留，不为 USD 垫资绕开。命令不调用任何外部付款客户端。
...
| 真实渠道/部署 | 未运行；未连接生产数据库、执行正式迁移、授权、垫资、Slash写入或刷卡 |
...
| 016迁移、最小权限、独立审批人与Worker | 工程/授权运营，上线前 | 本地实现，未部署或配置 |
