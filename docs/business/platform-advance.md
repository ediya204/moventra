...
| 类型 | 已实现账务流程 | 边界 |
| --- | --- | --- |
| 平台垫资 | 待审核 → 独立批准 → 清算分户到钱包 → 完成 | 不证明平台外部资金已经到账或形成正式应收科目 |
| 线下到账补录 | 凭证申请 → 独立批准 → 钱包入账 | 由人员核实真实到账且尚未入账；尚无银行自动核验或跨系统经济事项去重 |
...
| 项目 | 本批实现与验证边界 |
| --- | --- |
| 起终点 | 运营查询用户 → 创建申请 → 独立复核 → 持久 Worker 记账 → 两端查询原单及余额；真实外部付款由人员另行执行 |
...
| 权限 | 运营 MFA＋独立 read/create/review/execute 授权，scope 为具体客户或全局；禁止自审与给运营本人个人账户操作；客户端本人只读 |
...
权限表 `manual_funds_grants` 默认无授权，普通 admin 身份不自动获得资金权限。019 的有效全局管理员通过 `effective_manual_funds_grants` 继承全局动作权限；仍要求 MFA、禁止自审、禁止操作本人个人账户。其他运营人员按客户范围独立授权，不能为了开启按钮扩大授权。
...
| 016迁移、最小权限、独立审批人与Worker | 工程/授权运营，上线前 | 后续发布已安装016/019；有效全局权限沿用019。人工执行待本轮发布核验，独立审批人须实际具备授权 |
| 正式 Blnk、资金池、存量卡占款、转卡与消费闭环 | 工程/持卡人，真实刷卡前 | 尚待受控验收，本批未调用真实金融写接口 |

上线验收后，授权人员可搜索指定邮箱，经人工入金“平台垫资”提交一次 10,000.00 USD，独立审核并确认分录。钱包入账不自动给所有卡加额；按实际卡归属与资金流程分配，并由持卡人完成小额刷卡验证。
...
沿用 FLOW-PLATFORM-ADVANCE-001 的页面、业务身份、接口、权限和订单状态。本轮基线 main `9f747ab`，修改正式资金服务选择、API 内持久订单恢复及016依赖检查；共享工作区的开卡页面与测试不在本轮范围。目标为人工 USD 申请、独立审核、后台记账恢复及两端同源查询，不代替用户创建订单或执行历史文档中的垫资计划。
...
- API 启动及 `/readyz` 核对001/006/016/019校验和，不自动迁移。显式开启后，API 每5秒恢复现有 reserving/processing/releasing 订单，沿用订单锁、独立复核和固定账本 reference；没有待处理订单时不创建分录。
...
验收：本轮隔离PG验证正式模式下开关关闭/prepare拒绝、迁移校验错误拒绝、MFA、自审拒绝、创建幂等、独立审核、Worker恢复和重复运行仅记账一次、生产不健康与pilot拒绝；原人工生命周期覆盖出金预占、失败释放及未知恢复。具体运行结果同步于当前状态。真实登录、真实Blnk、渠道验证与部署未执行；用户已确认正式后台；发布核验进行中。
