...
## FLOW-CLIENT-OVERVIEW-001：生产首页数据一致性（2026-09-19，本地修复）

| 项目 | 本批范围与实现 |
| --- | --- |
| 目标及范围 | 登录客户从首页查看正式钱包、最近资金订单、本人授权卡片，并下钻现有详情；本批仅客户端只读展示，不新增金融执行 |
| 基线 | Moventra main 5e55451；工作区另有后端全局运营权限及后台改动，全部保留且不纳入本批发布 |
| 差异与兼容 | 生产旧标签页仍运行测试钱包页面；最新首页仍有占位统计与独立基础账户空列表。首页改读正式 crypto 和 card-projections；基础账户/交易保留在 /portal/accounts |
| 页面关系 | /portal、/portal/transactions → /portal/funds/history → /portal/funds/orders/:id；首页卡片预览 → /portal/cards/:id?connection=… → 对应来源卡片列表；充值到卡直接进入 /portal/funds/fiat |
| 业务身份 | 会话个人 customerId；资金订单 customerId 必须匹配；卡片身份按 customer、connection、cardId 隔离 |
| 数据依据 | 正式 live 钱包与订单沿用资金中心 crypto 查询；卡片为客户已授权来源投影，非余额或可执行资金 |
| 接口链 | ProductionWallet → cryptoRequest → 同域 client-api 网关 → 既有 Go crypto handler；CardOverview → liveGet → 网关 → 既有客户 card-projections handler → 授权投影查询；后端不修改 |
| 状态及操作 | 加载、失败、无记录、币种未开通、余额核对中分别显示；失败支持重试，不以零或空记录代替失败；只读查询不产生订单 |
| 跨端变化 | 首页与资金中心共用正式查询，钱包每15秒在可见页重查；卡片进入首页或点击刷新重查；不触发渠道同步。跨端真实浏览器复验未执行 |
| 权限 | 沿用服务端主体授权；前端拒绝 shadow、错误客户及订单归属不一致；切换客户丢弃旧响应 |
| 查询覆盖 | 资金最近5笔，全部记录入口使用现有分页；卡片各来源总数取服务端total，预览前5张，不拿局部页面计算全量状态分布或消费趋势 |
| 版本恢复 | 每60秒及返回可见页核对线上HTML入口版本，发现变化提示手动更新，不自动刷新资金表单。已经打开的旧版本仍需首次浏览器刷新才能取得此功能 |
| 验收 | E01/E03订单和卡片链接、E06错误恢复、E08覆盖、E09金额精度及跨客户响应隔离由新增组件测试验证；服务端越权与真实浏览器未重新验收 |
| 待定决策 | 全量消费趋势及状态聚合API不在本批，无来源不推算；独立后续需求确认，负责人待分配 |

设计及本地实现完成。`pnpm test` 140项通过（包含4项新增组件回归），客户端类型检查及生产构建通过。测试使用隔离响应，不代表真实渠道重新验收。没有生产迁移、部署或真实资金写入；线上仍需发布本批客户端后验收。原源码与测试钱包历史数据保留。


## FLOW-FUNDS-OTC-AUTOQUOTE：自动报价与成交（2026-09-19，本地）

| 项目 | 本批范围 |
| --- | --- |
| 目标/基线 | 共享工作区 main 14fe215 的 OTC UX 增量；保留已有卡片上下文、导航和国旗改动，不修改其他资金业务。 |
| 页面 | `/portal/funds/exchange`（兼容 `/portal/crypto/exchange`）输入卖出数量 → 自动买入报价 → 成交 → 原订单详情 → 返回原页面。 |
| 身份/数据 | 当前 session 的个人 customerId、服务端 quoteId、报价币种/金额/配置版本/有效期，正式钱包独立 USD/USDT 最小单位。 |
| 接口链 | OtcExchange → cryptoRequest → 原同域网关 → Go cryptoAPI → cryptofunds otc_quote 保存60秒报价；点击成交经父组件既有 otc_order 与幂等恢复链。报价不预占、不扣款。 |
| 状态/操作 | 停止输入400ms自动报价，金额/方向/客户/配置变化立即使旧报价不可用；只接受匹配请求的报价。过期更新、超时/失败可重试；成交前再次校验有效期、能力、余额和费用。 |
| 权限/恢复 | 服务端客户归属与能力检查不变；输入与报价不自动成交。成交仍保存原请求编号，未知结果冻结新操作并核对原请求；按钮不声称已经到账。 |
| 跨端 | 原订单查询/账务处理/审计不变，不新增渠道调用或后台政策。报价量因输入增加，采用400ms防抖、仅可见页查询及每次过期刷新。 |
| 验收 | E01/E02 主流程和订单导航；E06 报价失败/超时恢复；E07 延迟旧响应、重复成交及未知请求；E09 两币种精度、费用和余额校验。仅本地合成数据，不做真实成交。 |
| 待定决策 | 无新增业务政策；生产发布与真实账户验收未执行。 |

设计及本地实现完成。资金组件15项针对性测试和153项前端全量回归通过，两端类型检查与生产构建通过。隔离浏览器核对1280、390、320px布局，验证自动报价、USD精度、余额不足、报价失败重试及成交结果不确定时冻结/沿用原请求；无运行时错误。预览仅生成合成报价，订单写入关闭，没有真实成交、渠道复验或生产部署。


## 2026-09-19 卡片充提开发缺口核验

用户要求继续开发钱包转卡/卡转钱包。本节为本次源码核验与实施拆分，不把开关配置或已有模拟验收当作真实资金验收。

已有链路：`cryptofunds/cards.go`报价与分项费用、`commands.go`报价过期/归属/幂等订单、`card_worker.go`钱包或卡分户预占、渠道限额回读、账本结算、手续费及未知状态保留。前端固定卡片与方向，详情及订单返回上下文已实现。无需重新写一套转账表单或订单系统。

| 优先级 | 需要补齐的代码 | 当前证据与交付要求 |
| --- | --- | --- |
| P0 | 统一开卡与资金中心的卡分户适配及唯一映射 | `issuing/engine.go`使用`card_<orderID>`及自身balance管理；`cryptofunds`使用`ledger_accounts`/`funds_cards`。新开卡详情projection映射只解决导航，不完成资金账户桥接。必须明确唯一记账入口，接入首充结果，禁止重复建余额或重复消费入账。 |
| P0 | 新卡接入与存量卡期初流程 | `EnrollZeroCard`及CLI仅允许额度0、预占0、无交易的新卡，不能接管有历史的卡。需服务端归属复核、受控申请/审核、来源证据、期初科目和幂等激活；缺证据保持未开通，不按尾号推断。 |
| P0 | 渠道资金执行资格和分项不可用原因 | 生产`ProductionFromEnv`未构造CardProvider，`CheckProduction`明确拒绝Cards非空，必须先扩展生产profile并保持TRC20/OTC独立健康；通用live路径的`FUNDS_SLASH_CERTIFICATION_FILE`要求独占控制、collective额度语义、完整授权覆盖、恢复、同步生效及回读一致等证据；`CanOperate`多数失败统一提示待核实。需接入正式验收结果/版本/撤销及按方向的原因码，分别报告映射、期初、费用、余额、占用、渠道和对账状态。不能伪造认证JSON。 |
| P1 | 按卡增量同步、授权生命周期及公平调度 | `ReadCard`每卡两遍扫描交易列表，`SyncCards`每轮最多20卡按observed_at排序；持续失败卡会反复占用队列。需按account/card服务端过滤、持久游标/补查边界、失败退避与公平任务队列，消费/退款幂等与授权释放各自闭环，避免把两次相同分页当作完整授权覆盖证据。 |
| P1 | 充提执行恢复和运营异常工作台 | 已有unknown及账本幂等恢复；需补带当前归属/资格复检的受控恢复命令、渠道前后额度及账本效果证据、冲突进入人工调查、对账差异与审计查询。回读确认失败才能释放预占，禁止人工直接标成功或盲目重复写渠道。 |
| P1 | 统一资金记录和端到端验收 | 将开卡首充、后续充提、手续费、消费/退款及预占分别关联同一内部卡。覆盖跨客户/连接、并发消费时提现、重复提交、渠道成功但本地超时、重启恢复、费用变化与期初接入重复执行。最后用明确选定卡片/小额分别验收两个方向。 |

实施顺序：先做统一账户适配和只读就绪报告，再接新卡/存量卡接入，随后完善同步与恢复，最后进行指定卡片小额双向验收。钱包余额、卡内部可用资金和渠道消费限额必须分别核对；这里的限额更新只在已认证的独占管控模式中作为执行手段，不是读取或捏造逐卡余额。

本轮生产配置只读核验：API已有SLASH_API_KEY；尚未配置FUNDS_SLASH_CERTIFICATION_FILE。本次没有创建funds_cards期初、修改真实额度或转移资金。

## FLOW-FUNDS-TRANSFER-STYLE：充值与提现参考图布局（2026-09-19，本地未部署）

| 项目 | 本批内容 |
| --- | --- |
| 目标及范围 | 正式客户端 USDT 充值、提现采用绿色三步线、白色表单卡、右侧须知、底部近期记录；手机改为单栏，记录容器横向滚动 |
| 基线 | main `14fe215` 及已有共享工作区增量；保留 CustomerFunds/DepositAddress 等原有修改；本地合成预览 `MOVENTRA_PREVIEW_PORT=8873 node tests/frontend/client-layout-preview.mjs` |
| 页面关系 | `/portal/funds/deposit` → `?event=<id>` → 返回记录及5条分页；`/portal/funds/withdraw` → 原订单详情及带 returnTo 返回，更多进入 history?kind=withdrawal |
| 业务身份 | 沿用当前客户、充值通知 ID、资金订单 ID；不新增账户或来源关联 |
| 数据依据 | 充值地址/通知与正式 crypto 快照；参考图仅作样式依据，不复制示例金额、BNB 网络、费率、到账时间或安全合作方标志。通知不冒充订单；接口无手续费/到账金额字段时不虚构 |
| 接口链 | DepositAddress/CustomerFunds → cryptoRequest → 原同域网关 → deposit-addresses/crypto handler → 本人授权 → 原地址通知/订单存储；本批无接口变更 |
| 状态及操作 | 地址首次申请、复用、复制、切链清旧二维码原样保留；提现先查看费用，再显式发起，保留报价过期、能力禁用与原幂等请求恢复；费用确认移入第三步 |
| 跨端变化 | 无新增后台动作；沿用5秒查询同一订单/通知，后台审核规则不变 |
| 权限 | 原客户归属及服务端能力校验；不新增金融开关、网络或真实执行授权 |
| 验收 | 两端类型检查和构建通过；相关组件回归15项通过（排除现有“人工记录导航”断言）；新增提现精确输入/报价绑定/切链作废测试通过。浏览器合成数据检查桌面双栏、390px单栏、ERC20切换清除地址；没有真实扫码、系统剪贴板或真实登录验收 |
| 全量限制 | pnpm test 出现现有导航两项失败：client-workspace 的导航分组与 customer-funds 的人工记录入口断言；后者失败未卸载导致定时器不退出，全量不认定通过。没有为样式任务改回共享导航 |
| 待定决策 | 无新增业务政策；渠道开放继续由原能力配置决定 |

设计、本地实现完成；自动化及浏览器范围如上。真实渠道、生产迁移、部署和真实资金操作均未执行。共享视觉组件为 `CryptoTransferLayout`，充值和提现仅重排展示，金额及订单状态不变。

### 2026-09-19 资金中心移除法币充提（本地未部署）

按用户要求，资金中心删除独立“法币充提”分区，首页同步删除指向该页的“充值到卡”快捷入口。旧 `/portal/funds/fiat`、`/portal/funds/fiat-deposit` 及 `?tab=fiat`（含 fiat-deposit）以 replace 返回资金总览，历史 crypto 基础路径同样兼容。FLOW-FUNDS-TRANSFER-STYLE 页面范围据此更新。

本次为导航与独立页面移除，卡片详情上下文的充值/退回钱包、历史订单查询及服务端权限保持原样；旧路由跳转不发起报价或订单。原页面接口链不新增接口，客户归属、审核和幂等规则不变。自动化覆盖旧链接返回总览且无写请求、卡片上下文仍可独立报价；真实渠道、生产部署均未执行。

本轮验证：两端类型检查及构建、文档检查通过；27项相关组件测试通过，排除上一节已记录的两项既有导航断言。合成浏览器已确认访问旧 fiat 链接后 URL 为 `/portal/funds`，分区不再包含法币充提。未进行真实资金操作。
