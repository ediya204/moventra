...
日期：2026-09-13。状态：本地实现，未部署。FLOW-AUTH-002。
...
| 待定决策 | 未授权发布；混合旧身份迁移明确报错，不自动删除客户归属或扩大权限 |
...
上线需经授权：备份与混合身份检查 → 迁移 004 → API → 两端 Worker。新前端遇到旧 API 缺少 role 会拒绝准入，不能倒序发布。未连接生产数据库、未执行真实 Firebase 测试、未部署。
