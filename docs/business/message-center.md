...
更新日期：2026-09-19。**本地实现，隔离闭环已验证；未部署、未启用生产消息订阅或发送。** 用户在设计后授权“实施”。基线为 main `14fe215` 加共享工作区增量，保留并行卡片、资金、登录及页面改动。
...
增量迁移 [020_messages.sql](../../services/api/internal/database/020_messages.sql) 不修改已应用迁移。实际结构：
...
默认所有运行能力关闭，不自动连接生产或执行DDL。独立迁移命令（从services/api运行）为 `go run ./cmd/api migrate-messages`，校验001–019依赖及checksum；API启动不自动迁移。仅在明确授权的目标库执行。
...
| 真实渠道/部署 | 未执行；未连接生产库、生产020未迁移、消息权限/订阅未启用、未发送真实客户消息 |
