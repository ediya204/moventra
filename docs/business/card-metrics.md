...
状态：已实现并部署，020迁移完成，指定账户23张卡已初始化；近30天真实数据覆盖完整。更早历史额外28笔、69个任务全部完成，结果见[发布记录](../../deploy/2026-09-19-card-metrics-release.md)。用户确认先对指定邮箱正式归属卡建立基线，再将历史和通知统一落库；不得生成资金期初或重复扣款。
...
- CLI已上线并核验23张正式归属卡；020定向迁移和真实回填已执行。近30天23个窗口均完整、8笔来源记录；21张卡返回可消费额度、2张来源未提供。未写资金账本或调用真实金融写接口。
- 真正登录页面已核验尾号3475的14.85 USD可消费额度、105.15 USD近30天消费，以及相同精确区间内1笔OPENAI消费明细。发布后修复重复刷新造成加载闪动，保留父页面既有数据库刷新。未制造真实消费或退款以触发新Webhook。
...
## 首页每日趋势（本地候选，2026-09-19）

客户端按选中卡的完整指标窗口分页读取既有消费构成接口，以UTC日和BigInt分金额汇总。读完后核对金额与metrics一致才补齐无消费日期为零；不完整、失败或不一致不绘图。仅USD分口径，不改变接口与渠道同步。见[首页流程](../frontend/client-workspace-layout.md)。
