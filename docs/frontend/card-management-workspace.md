...
文档初次发布仅包含实施记录与对账规划。随后 [卡片详情导航发布](../../deploy/2026-09-07-card-detail-navigation.md) 同步了后台卡片工作台前端和卡内交易 GET 白名单；下文 Node 接口、迁移与客户端解冻代码不包含在该次同步中。下文测试仍为此前本地证据，仓库与本地预览的能力不能视为完全一致；生产只发布渠道只读页面的导航修复。
