...
当前代码口径需区分两套本地模型：旧本地资金工作台 `crypto-finance.mjs` 已采用默认 `1 USDT = 0.99 USD` 的可配置固定比例，新报价无倒计时、确认时校验价格版本，两个方向使用同一比例反向计算；调价后未确认旧报价失效，已成交记录不追改。旧 Portal 的 `PORTAL_QUOTE_POLICY` 仍保留 60 秒及 `0.997/1.001` 两个演示方向，历史 `fn_quotes` 也仍按过期时间校验。下方历史正文中的 60 秒不能作为全部现行报价规则。商业费率方案的 `mg_fees` 继承尚未接到独立资金工作台的固定测试计费。DEV `/approvals` 是开户审核列表，并在 Slash DEV 模式提供独立卡操作审批入口；数字货币出金仍在 `/finance/withdrawals` 审批。完整页面、审批、精度和资金/来源分区见 [现行业务说明](../business/funds-and-operations.md)。
