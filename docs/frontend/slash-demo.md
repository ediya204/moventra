...
- Demo凭据：`demo@moventra.local` / `demo-only`。仅本地模拟认证；不代表真实SSO或API授权。刷新会清除内存会话。
...
`NODE_ENV=production`、生产Moventra_ENV及已设置的非loopback数据库/渠道/代理环境变量会在打开存储前拒绝。该命令没有真实Slash网络客户端。普通模式原有远程代理不用于隔离Demo；新模式固定代理127.0.0.1:8862。端口冲突时启动失败，不静默换端口。不要将Demo静态构建当成生产API接入。
