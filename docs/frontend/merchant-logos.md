...
- 发布方案：main 含其他未发布前端增量，分别从两端已发布提交生成隔离候选，只应用本次品牌修复；部署结果见下方本次发布证据。
- 待定业务决策：无。能力范围为商户图标展示，不代表渠道确认品牌身份。

### 本次发布证据

- GitHub main 修复提交：f4e77f8。两端发布内容分别为 a2fa1f6 / c283b71 基线加该提交的 merchantBrand.ts；测试文件也复制到隔离目录，分别 6 项品牌测试和对应生产构建通过。未发布 main 的其他前端增量。
- 发布前分别重建两端原版本，入口 JS 文件名与线上一致；发布后含品牌映射的线上 JS 与隔离产物逐字节一致，两个品牌的外部 PNG 请求成功。
- Cloudflare 后台：208f4bf4-06a0-4238-b139-cfadb0b61cdc；客户端：e934dc73-9b29-4f0e-a0f3-c40c9652430b。标签均为 logo-f4e77f8；发布说明注明各自原始基线。
- 回退版本：后台 0f8213dd-5833-4c57-bb5e-beffa0f51fac，客户端 bdd6559d-ee8b-4f33-b411-1e8524d2b09b。没有 API 发布、数据库迁移或真实金融写入。
- 设计、本地实现、自动化及发布已完成；浏览器本人登录后的实际交易行验收未执行，不把静态资源核对视为该项通过。文档证据提交在修复提交之后，不包含额外运行代码。
