...
2026-09-18 Cregis 已确认采用 WaaS，新增签名/回调校验、只读流水客户端及独立核验命令；本地 TLS 模拟测试通过。用户确认凭据已配置到 Render，Dockerfile 已补充核验命令打包，但平台仍运行旧提交 a2fa1f6。尚无真实渠道验证、HTTP 入口、来源存储或正式资金功能，见 [Cregis 接入](integrations/cregis.md)。
