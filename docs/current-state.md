...
更新日期：2026-09-18。客户端开卡页面发布代码为 `92cad84`，API/Worker连接准备已发布 `2b4a905`，本批生产目录配置及应用发布证据见下方和[发布记录](../deploy/2026-09-18-client-issuing-release.md)。其他专题按各自记录的日期与验证范围解释，不以本批开卡验证覆盖。

## 客户端开卡与八个 BIN 发布（本次）

014 已于香港17:47迁移生产；17:59八个BIN上架，开卡费10 USD、最低首充20 USD、最低合计30 USD。API代码92cad84于18:05上线，两端Cloudflare已发布；116项前端测试、17项官网测试、两端类型检查/构建、隔离PG race及28项生产探测通过。浏览器仅验证未登录入口，隔离资金闭环证据保留。版本及边界见[本次发布记录](../deploy/2026-09-18-client-issuing-release.md)。真实金融执行尚未开放：独立账本TLS连接与只读开卡Worker已部署并验证，真实渠道验收清单仍缺失，供应商保持暂停；见[连接部署记录](../deploy/2026-09-18-issuing-preparation.md)。
...
2026-09-18 资金中心四流程已发布：开户完成后首次进入充值页才申请 Cregis 客户专属地址；双链与正式账本保持待验收、未激活。见[资金中心](business/funds-center.md)。
...
地址管理与正式账本执行解耦；复用已建TRC20地址、客户唯一持久绑定、独立回调和最近通知/详情接口。正式资金继续关闭。详见[流程及部署边界](business/deposit-address-integration.md)。
