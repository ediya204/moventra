...
017首批已上线，23张卡历史核验14正常、9暂停，原发布记录见[同步发布](../deploy/2026-09-18-card-state-sync.md)。用户随后确认取消定时补查：018与两端卡片操作已本地实现，改为API命令、Webhook及手动核对驱动；无事件不按时间标记过期。保留正式归属、历史交易；不设置每卡10美元、不新增CVV。新批次的测试及部署边界见[事件驱动发布](../deploy/2026-09-18-card-controls.md)。
