...
| Slash 投影 | 已导入基础资料、客户范围隔离、项目钱包逐卡分配；已启用卡片当前状态由通知和周期回查更新 | 23张正式归属卡已全部真实核验，两端共同读取状态；交易仍为导入范围，不代表完整历史或可用资金。见[同步发布](../deploy/2026-09-18-card-state-sync.md) |
...
本地全用户余额查询、人工订单/独立审核/预占/恢复和客户端记录已实现，无用途分类；016迁移及独立权限默认不激活真实资金。发布证据见[人工资金流程](business/platform-advance.md)。
