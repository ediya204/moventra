  ...
  privacy: {
    ...
    sections: [
      ...
      section('您提供的信息', 'Information you provide', copy(
        '官网服务需求表包含姓名、工作邮箱、意向服务和需求描述。点击“提交留言”后，表单通过 Cloudflare 处理并发送至 info@moventra.me，用于回复本次咨询。您的邮箱会作为回复地址；IP 地址用于限制提交频率。注册预览页面仅在浏览器中检查格式，不创建账户或提交填写内容。密码找回会将您提供的邮箱发送至 Google Firebase，以处理密码重置邮件请求。',
        'The service brief asks for your name, work email, service of interest, and requirements. When you select “Send inquiry”, Cloudflare processes the form and emails it to info@moventra.me so we can respond. Your email is used as the reply address; your IP address is used to limit submission frequency. The registration preview validates input in the browser without creating an account or submitting that input. Password recovery sends the email address you provide to Google Firebase to process a password reset email request.'
      ), copy(
      ...
      section('信息接收方与跨境处理', 'Recipients and international processing', copy(
        '根据您使用的功能，必要信息可能由 Cloudflare（网站分发、安全及留言邮件发送）、Spacemail（咨询邮箱收信）、Google Firebase / Google（身份验证与账户邮件）、Render（业务 API 托管）、Iconify（图标资源）及参与已约定交付的服务提供方处理；仅为回应请求、交付服务或履行法律义务而披露相关信息。第三方平台会按自身政策处理您直接提供给它们的信息。当前官网没有配置出售个人信息或基于跨站行为进行广告定向的功能。',
        'Depending on the features you use, necessary information may be processed by Cloudflare (website delivery, security, and inquiry email sending), Spacemail (inquiry mailbox hosting), Google Firebase / Google (authentication and account emails), Render (business API hosting), Iconify (icon resources), and providers involved in agreed delivery. Relevant information is disclosed to respond to requests, deliver services, or comply with legal obligations. Third-party platforms process information you provide directly to them under their own policies. The current website has no configured feature for selling personal information or targeting advertising based on cross-site behavior.'
      ), copy(
  ...
  terms: {
    ...
    sections: [
      section('服务提供方与适用范围', 'Provider and scope', copy(
        '本条款由 Moventra Technology Inc（“Moventra”“我们”）提供，适用于本官网及服务咨询。具体采购或开通服务须由有权代表双方的人员确认订单或服务协议；其中针对具体服务的约定优先适用，但不得排除适用法律规定的强制性权利。浏览网页或提交留言本身不构成付款授权、订阅开通或订单成立。',
      ...
      section('预览功能与账户', 'Preview features and accounts', copy(
        ...
        'The inquiry form sends your message to our inquiry mailbox. A successful submission means the email service accepted the request, without guaranteeing immediate delivery or a reply within a particular time. The registration page is a format preview; password recovery submits a reset email request to Firebase. Post-login profile completion can create a website user but does not automatically activate business or financial accounts. Real accounts are for authorized users. Signing in does not automatically grant access to other customers or business data. Provide accurate information, protect credentials, and act only for yourself or an organization you are authorized to represent. Demo pages, sample amounts, and simulated actions do not prove funds availability, completed payment, or activated services.'
      )),
      ...
      section('内容与知识产权', 'Content and intellectual property', copy(
        '官网的品牌、设计和原创内容受适用知识产权法律保护。您可以为了解服务而正常浏览和提交服务咨询，但不得冒充 Moventra 或未经授权将网站内容用于误导性推广。您保留对所提交材料的权利，并应具备为约定服务提供这些材料所需的授权；我们对材料的使用限于处理请求和约定交付所需范围。',
        'Our branding, design, and original website content are protected by applicable intellectual property laws. You may browse to evaluate services and submit an inquiry, but may not impersonate Moventra or use site content for unauthorized misleading promotion. You retain rights in submitted materials and must have the authority to provide them for the agreed service; our use is limited to responding to your request and performing agreed delivery.'
      )),
  ...
  cookies: {
    ...
    sections: [
      ...
      section('登录会话与表单', 'Sign-in sessions and forms', copy(
        '当前前端登录令牌保存在页面内存中，不由前端写入 Cookie、localStorage 或 sessionStorage；刷新或退出会清除该前端会话。需求表和注册预览内容不会由页面写入持久化存储；提交留言会将内容发送到我们的邮箱，频率限制不依赖追踪 Cookie。密码找回会提交邮箱到 Firebase，但不由页面保存该邮箱。浏览器自身的自动填充和密码保存由您的浏览器设置控制。',
        'The current frontend sign-in token is held in page memory and is not written by the frontend to cookies, localStorage, or sessionStorage; refreshing or signing out clears that frontend session. Inquiry input and registration previews are not written to persistent storage by those pages. Submitting an inquiry sends its contents to our mailbox; submission rate limiting does not use tracking cookies. Password recovery submits an email address to Firebase but does not persist that address in the page. Browser autofill and password saving are controlled by your browser settings.'
      )),
      ...
      section('如何管理', 'How to manage storage', copy(
        '您可以在浏览器的隐私或网站数据设置中查看、阻止或清除本网站的 Cookie 和本地存储。清除后语言偏好会丢失；阻止认证或安全服务所需的 Cookie 可能影响相应服务。退出登录可结束当前前端会话。删除网站数据不会删除已经提交到我们邮箱的留言，也不等于向我们提出删除服务端个人信息的请求；后者请参阅隐私政策。',
        'Use your browser’s privacy or site data settings to inspect, block, or clear this site’s cookies and local storage. Clearing removes your saved language preference; blocking cookies needed by authentication or security services may affect those services. Signing out ends the current frontend session. Clearing site data does not delete inquiries already submitted to our mailbox or request deletion of server-side personal information; see the Privacy Policy for those requests.'
      )),
