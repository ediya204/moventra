...
export default function Website() {
    ...
    const services = [
        ...
        { title: t("订阅卡"), icon: 'solar:card-2-bold-duotone', description: t("面向 Netflix（奈飞）、Google 等平台的订阅需求，提供订阅卡方案咨询。"), items: [t("Netflix（奈飞）订阅场景"), t("Google 等数字服务订阅"), t("适用平台与地区确认")] },
    ...
    const solutions = [
        ...
        { name: t("数字订阅用户"), title: t("影音与数字服务订阅"), description: t("适合有 Netflix（奈飞）、Google 等平台订阅需求的个人与团队。"), services: [t("订阅卡")], tasks: [[t("订阅需求"), t("确认所需平台、订阅类型与使用地区。")], [t("方案确认"), t("核对订阅卡适用范围、使用条件与订阅周期。")], [t("使用指引"), t("按确认的方案了解订阅流程与续订注意事项。")]] },
    ...
    const questions = [
        [t("可以单独购买一项服务吗？"), t("可以。四类服务均可单独咨询，也可以根据业务需求组合。具体服务内容和交付范围会在合作前确认。")],
        [t("AI 订阅和订阅卡有什么区别？"), t("AI 订阅侧重 AI 工具的选型与订阅方案；订阅卡面向 Netflix（奈飞）、Google 等平台的数字服务订阅需求。具体适用平台、地区和使用条件会在确认方案时说明。")],
    ...
    <Container component="footer" maxWidth="lg" sx={{ pt: 7, pb: 3 }}><Grid container spacing={4} sx={{ mb: 6 }}><Grid item xs={12} md={6}><BrandLogo width={170}/><Typography variant="body2" color="text.secondary" sx={{ mt: 2, maxWidth: 280 }}>{t("广告营销、AI 订阅、订阅卡与云服务。")}</Typography></Grid><Grid item xs={6} md={3}><Stack spacing={1.5}><Typography variant="subtitle2">{t("服务")}</Typography>{services.map(item => <Link key={item.title} component="button" onClick={() => choose(item.title)} variant="body2" color="text.secondary" sx={{ textAlign: 'left' }}>{item.title}</Link>)}</Stack></Grid><Grid item xs={6} md={3}><Stack spacing={1.5}><Typography variant="subtitle2">{t("账户与帮助")}</Typography>{[[t("登录"), '/login'], [t("注册账户"), '/register'], [t("找回密码"), '/forgot-password'], [t("常见问题"), '#faq']].map(([label, href]) => <Link key={href} href={href} variant="body2" color="text.secondary">{label}</Link>)}</Stack></Grid></Grid><Divider /><Typography variant="caption" color="text.secondary" sx={{ display: 'block', pt: 3 }}>© {new Date().getFullYear()} Moventra. All rights reserved.</Typography></Container>
  </Box></PublicTheme>;
}
