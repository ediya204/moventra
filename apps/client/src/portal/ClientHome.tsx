          ...
          {security ? (
            ...
            <Stack spacing={3}>
                  ...
                  <Paper variant="outlined" sx={{p:2}}><Stack direction={{xs:'column',sm:'row'}} gap={1.5} alignItems={{sm:'center'}}>
                    ...
                    <Box sx={{...workspaceGrid,flex:1,gap:1}}>
                      {[["充值 USDT", "solar:wallet-money-linear"], ["兑换 USD", "solar:refresh-linear"], ["充值到卡", "solar:card-transfer-linear"], ["申请新卡", "solar:card-linear"]].map(([label, icon]) => (
                        <Button key={label} component={Link} to={label === "申请新卡" ? "/portal/cards/new" : label === "充值 USDT" ? "/portal/funds/deposit" : label === "兑换 USD" ? "/portal/funds/exchange" : "/portal/funds/fiat"} disabled={!enabled} fullWidth startIcon={<Icon icon={icon} width={20} />}>{label}</Button>
                      ))}
                    </Box>
                  </Stack></Paper>
              ...
              {pathname === "/portal/messages" && <Paper variant="outlined" sx={{p:{xs:3,md:5}}}><Stack alignItems="center" spacing={2} sx={{maxWidth:520,mx:'auto',textAlign:'center'}}>
                ...
                <Stack direction={{xs:'column',sm:'row'}} gap={1}><Button component={Link} to="/portal/funds/history" variant="outlined">查看资金记录</Button><Button component={Link} to="/portal/card-orders">查看开卡订单</Button></Stack>
              </Stack></Paper>}
