 ...
 return <Stack spacing={3}>
  <Box sx={{display:'grid',gridTemplateColumns:{xs:'minmax(0,1fr)',md:'minmax(0,0.95fr) minmax(0,1.2fr)'},columnGap:{xs:3,md:4},rowGap:1.5}}>
   ...
   <Stack spacing={2.5} sx={{minWidth:0,gridColumn:{md:2},gridRow:{md:1},borderLeft:{md:1},borderColor:{md:'divider'},pl:{md:4}}}>
    <Stack direction="row" alignItems="center" justifyContent="space-between"><Typography variant="h6">资金情况</Typography><Button size="small" onClick={()=>setRetry(n=>n+1)} startIcon={<Icon icon="solar:refresh-linear"/>}>刷新资金</Button></Stack>
    <Box sx={{bgcolor:'#F4F6F8',borderRadius:2.5,p:{xs:2,sm:2.5}}}>
     ...
     <Typography aria-label="剩余额度待接入" sx={{fontSize:32,lineHeight:1.3,fontWeight:700,mt:.5,mb:2,fontVariantNumeric:'tabular-nums'}}>—</Typography>
     <Box aria-label="额度使用进度暂不可用" sx={{height:6,bgcolor:'#DFE3E8',borderRadius:3,mb:1.5}}/>
     <Box sx={{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:2}}>
      {[['可用额度','总额度尚未接入'],['已用额度','同周期已用额度尚未接入']].map(([label,reason])=><Box key={label}><Typography variant="caption" color="text.secondary">{label}</Typography><Typography variant="subtitle2" title={reason} sx={{mt:.25}}>待接入</Typography></Box>)}
     </Box>
     <Typography variant="caption" color="text.secondary" display="block" mt={1.5}>额度数据暂未接入，使用进度与剩余额度待核实。</Typography>
    </Box>
