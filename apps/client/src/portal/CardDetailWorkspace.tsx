...
export default function CardDetailWorkspace({customerId,connection,card,reload,onRefresh}:{customerId:string;connection:string;card:Card;reload:number;onRefresh:()=>void}){
 ...
 const metricRefresh=async()=>{setMetricBusy(true);setMetricError('');try{await liveCardSync(`/client-api/v1/customers/${customerId}/card-projections/${connection}/cards/${card.id}/metrics-sync`);onRefresh()}catch(e){setMetricError(authMessage(e))}finally{setMetricBusy(false)}};
 ...
 return <Stack spacing={3}>
  ...
  <Paper variant="outlined" sx={{p:2}}><Stack gap={1}>
   <Typography>可消费额度：{cardMetricDisplay(card.metrics,'available').value}</Typography>
   <Typography variant="caption" color="text.secondary">{cardMetricDisplay(card.metrics,'available').help}</Typography>
   <Typography>近 30 天消费：{cardMetricDisplay(card.metrics,'spending').value}</Typography>
   <Typography variant="caption" color="text.secondary">{cardMetricDisplay(card.metrics,'spending').help}</Typography>
   <Stack direction="row" gap={1}><Button disabled={card.metrics?.coverage!=='complete'} onClick={showSpending}>查看消费明细</Button><Button disabled={!card.metrics||metricBusy||card.metrics.syncState==='pending'} onClick={metricRefresh}>{metricBusy||card.metrics?.syncState==='pending'?'同步中…':'补同步交易与额度'}</Button></Stack>
   {metricError&&<Alert severity="error">{metricError}</Alert>}
  </Stack></Paper>
