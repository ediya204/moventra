...
卡片备注增量（2026-09-19，本地未部署）：`/portal/cards` 提供独立备注列，保留筛选/分页，点击备注编辑、Enter保存、Esc取消，失败保留草稿。正式持久化依赖API迁移023；8897预览仅使用当前会话合成数据。见[流程与验收](../../docs/business/customer-card-binding.md#flow-card-remark-001卡片备注2026-09-19本地未部署)。
...
首页按常用入口、横向钱包概览、独立消费分析、紧凑卡片列表、近期资金记录组织；桌面趋势图宽于辅助对比区，手机依序堆叠。以资产和流向图标辅助识别钱包与资金记录；卡片概览直接显示现有额度/近 30 天消费指标及详情入口。增加逐卡每日消费柱状图（完整分页并核对指标后绘制），右侧为当前所选卡片近30天消费前五商户及金额，共用完整明细和统计窗口。异常提醒限于最近记录，隐藏内部来源名称。未部署；完整流程与验收见[首页布局](../../docs/frontend/client-workspace-layout.md#flow-client-home-002首页概览与常用操作2026-09-19本地候选)。本机预览：`node tests/frontend/client-home-preview.mjs`，访问 `http://127.0.0.1:8868/portal`，仅使用合成数据。
...
交易菜单下新增 `/portal/fund-records`，非消费业务筛选、分页及详情深链；旧资金历史兼容。见[流程与验收](../../docs/business/fund-records.md)。

2026-09-19 申请页本地精简：紧凑产品列表、说明折叠与选卡/首充/支付渐进展示；费用与执行门槛不变，未部署。见[流程及验证](../../docs/business/client-card-issuing.md#flow-issuing-ui-002申请页精简2026-09-19本地未部署)。
...
2026-09-19：开卡流程预览：以 `MOVENTRA_PREVIEW_PORT=8897 node tests/frontend/client-home-preview.mjs` 启动，访问 `http://127.0.0.1:8897/portal/cards/new?previewIssuing=1`；支持选卡、合成首充报价与确认页，最终支付按钮禁用，订单写入拒绝。去掉参数可查看未开放状态。

开卡预览当前选卡后自动计算费用，修改初始卡余额后400ms更新；失败/过期可重算，最终支付继续禁用。本地实现未部署，见[开卡流程](../../docs/business/client-card-issuing.md)。

2026-09-19（本地未部署）：`/portal/security` 提供密码修改、邮箱验证变更、2FA绑定和替换的展开表单；实际身份变更需要 Firebase 确认。见[安全设置流程](../../docs/business/two-role-login.md#flow-auth-security-001账户安全设置2026-09-19本地未部署)。

账户设置入口已精简为直接访问 `/portal/security`，无“个人设置/我的账户”标签；旧 `/portal/settings` 自动转入安全页，旧 `/portal/accounts` 仅保留深链兼容。本地未部署。

2026-09-19 消费记录详情（本地未部署）：抽屉增加商户描述、MCC及商户名称/位置，复用既有投影并明确缺失值。18项专项及客户端类型/构建、边界检查通过，真实浏览器/渠道未复验。见[商户详情流程](../../docs/business/customer-card-binding.md#flow-tx-merchant-001消费详情商户信息2026-09-19本地未部署)。

### 单张开卡试运行（2026-09-19）

指定客户的钱包响应包含pilot范围，开卡页显示仅一张、BIN、20 USD固定首充及最多30 USD总额；服务端报价/提交/Worker重复校验。客户仍须独立勾选两项声明并自行提交；提交后自动处理，后台留记录，无逐单审批。试运行首充失败不显示再次补首充入口，查看原单核查。真实部署与未验证边界见[试运行记录](../../deploy/2026-09-19-issuing-pilot.md)。
