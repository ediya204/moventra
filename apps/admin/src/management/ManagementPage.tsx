...
export function ManagementAccess({
  ...
  useEffect(() => {
    ...
    window.addEventListener('moventra:management-session-expired', expired);
    return () => window.removeEventListener('moventra:management-session-expired', expired);
  }, []);
  if (!local)
    return (
      <Alert severity="info">
