...
export const productionNavigation = navigationGroups(true).map(group => ({
  ...group,
  items: [...(group.label === '资金与财务' ? [{label:'余额查询',path:'/finance/balances',icon:'solar:wallet-money-bold-duotone'}] : []), ...group.items].filter(item => item.path !== '/demo/scenarios').map(item =>
...
export function isProductionPath(path: string) {
  return ['/finance/balances', '/finance/crypto-flows', '/finance/withdrawals', '/finance/otc', '/system/cregis', '/pricing', '/reports', '/card-bins', '/finance/test-funds', '/workbench', '/onboarding', '/user-groups/users', '/cards', '/customers', '/system/channels', '/transactions', '/session?security=1'].includes(path);
}
