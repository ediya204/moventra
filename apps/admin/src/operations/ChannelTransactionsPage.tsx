...
function ChannelContent(){
 ...
 liveGet<Page<Tx|Card>>(`/admin-api/v1/channel-projections/${encodeURIComponent(connection)}/${id?'cards/'+encodeURIComponent(id):'transactions?'+q}`).then(x=>{if(active){setData(x);setPollError(false)}}).catch(e=>{if(active)setError(errorText(e))}).finally(()=>{if(active)setBusy(false)});return()=>{active=false}},[connection,filters,id,refresh,connections]);
 ...
 const show=async(row:Tx)=>{const token=++request.current;setSelected(row);setCard(undefined);setDetailError('');setDetailBusy(true);try{const detail=await liveGet<Page<Tx>>(`/admin-api/v1/channel-projections/${connection}/transactions/${row.id}`);if(token!==request.current)return;setSelected(detail.rows[0]);if(row.cardId){const linked=await liveGet<Page<Card>>(`/admin-api/v1/channel-projections/${connection}/cards/${row.cardId}`);if(token!==request.current)return;const c=linked.rows[0];setCard({...c,maskedCardNumber:(c.cardLast4||c.last4)?'•••• '+(c.cardLast4||c.last4):undefined})}}catch(e){if(token===request.current)setDetailError(errorText(e))}finally{if(token===request.current)setDetailBusy(false)}};
 const columns:GridColDef<Tx>[]=[
 {field:'merchant',headerName:'商户 / 交易',minWidth:250,flex:1,renderCell:p=><MerchantCell name={p.row.merchant} subtitle="卡片交易"/>},
 ...
 {field:'customerAssignment',headerName:'所属账户',description:'卡片分配的内部客户；不代表具体资金账户或资金所有权',width:210,sortable:false,renderCell:p=>{
  const owner=p.row.customerAssignment;
  return owner?.state==='assigned'&&owner.id?<Button component={Link} to={`/customers?${new URLSearchParams({customer:owner.id})}`} sx={{textTransform:'none'}}>{owner.name||'客户名称未提供'}</Button>:<Typography variant="body2" color="text.secondary">{owner?.state==='restricted'?'已分配 · 无查看权限':owner?.state==='unassigned'?'未分配':'归属未查询'}</Typography>;
 }},
 ...
 useEffect(()=>{if(!id||!connection)return;let active=true;const timer=setInterval(()=>{if(document.visibilityState==='visible')liveGet<Page<Card>>(`/admin-api/v1/channel-projections/${connection}/cards/${id}`).then(x=>{if(active){setData(x);setPollError(false)}}).catch(()=>{if(active)setPollError(true)})},15000);return()=>{active=false;clearInterval(timer)}},[id,connection]);
 return <DashboardLayout production><Stack gap={2.5}><Stack direction="row" justifyContent="space-between"><Box><Typography variant="h4">{id?'卡片详情':params.get('cardId')?'此卡交易流水':'卡交易流水'}</Typography><Typography color="text.secondary" sx={{mt:1}}>{id?'Slash · 卡片状态与管理':'Slash · 真实只读记录'}</Typography></Box><Button disabled={busy} onClick={()=>setRefresh(n=>n+1)}>刷新状态</Button></Stack>
 ...
 {id?busy?<PageSkeleton/>:current&&<Paper variant="outlined" sx={{p:3}}><Stack gap={2}><Typography variant="h5">{current.cardName||'卡名未采集'} · {(current.cardLast4||current.last4)?'•••• '+(current.cardLast4||current.last4):'尾号未采集'}</Typography><Typography>所属用户：{channelOwnerLabel(current)}</Typography><ChannelCardStatus status={current.cardStatus}/>{current.syncState&&<Button disabled={current.syncState==='pending'} onClick={()=>liveCardSync(`/admin-api/v1/channel-projections/${connection}/cards/${id}/sync`).then(()=>setRefresh(n=>n+1)).catch(e=>setError(errorText(e)))}>向渠道核对状态</Button>}<CardControls row={current} path={`/admin-api/v1/channel-projections/${connection}/cards/${id}/actions`} onRefresh={()=>setRefresh(n=>n+1)}/><Typography>创建时间：{utcTime(current.createdAtUTC)}</Typography><Typography color="text.secondary">启用、停用和注销以渠道确认结果为准；资金信息独立核对。</Typography><Button component={Link} to={`/transactions?${new URLSearchParams({connection,cardId:id})}`}>查看此卡交易</Button><Button component={Link} to={`/cards?${new URLSearchParams({connection,page:params.get('cardsPage')||'0',keyword:params.get('cardsKeyword')||'',status:params.get('cardsStatus')||''})}`}>返回全部卡片</Button></Stack></Paper>:
 ...
 <DataGrid onRowClick={(params,event)=>{if(!(event.target as HTMLElement).closest('a,button,input,select,textarea,[role="button"]'))void show(params.row);}} onCellKeyDown={(params,event)=>{if(event.key==='Enter'&&!(event.target as HTMLElement).closest('a,button,input,select,textarea,[role="button"]')){event.preventDefault();void show(params.row);}}} autoHeight rowHeight={56} columnHeaderHeight={40} rows={id?[]:(data?.rows as Tx[]||[])} columns={columns} loading={busy} disableRowSelectionOnClick disableColumnSorting disableColumnFilter paginationMode="server" rowCount={data?.total||0} paginationModel={{page,pageSize:20}} onPaginationModelChange={m=>{const q=new URLSearchParams(params);q.set('page',String(m.page));setParams(q)}} pageSizeOptions={[20]} localeText={zhCN.components.MuiDataGrid.defaultProps.localeText} getRowClassName={p=>transactionRowClass(p.row.status,p.row.detailedStatus)} sx={{...transactionRowStyles,border:0,'& .MuiDataGrid-row':{cursor:'pointer'},'& .MuiDataGrid-cell':{fontVariantNumeric:'tabular-nums',display:'flex',alignItems:'center',borderColor:'divider'},'& .MuiDataGrid-columnHeaders':{bgcolor:'grey.50'}}}/></Paper>}
 <LogoAttribution/>
