...
function ChannelContent(){
 ...
 const filters=JSON.stringify({keyword:params.get('keyword')||'',detailedStatus:params.get('status')||'',from:params.get('from')||'',to:params.get('to')||'',page});
 useEffect(()=>{let active=true;setBusy(true);liveGet<Connection[]>('/admin-api/v1/channel-projections').then(x=>{if(active)setConnections(x)}).catch(e=>{if(active){setError(errorText(e));setBusy(false)}});return()=>{active=false}},[refresh]);
 ...
 return <Box sx={{minHeight:'100vh',bgcolor:'background.default'}}><Box component="header" sx={{bgcolor:'background.paper',borderBottom:1,borderColor:'divider',px:3,py:2}}><Stack direction="row" justifyContent="space-between" alignItems="center"><BrandLogo/><Stack direction="row"><Button component={Link} to="/workbench">资金与运营</Button><Button component={Link} to="/transactions">卡交易流水</Button><Button component={Link} to="/session?security=1">身份与权限</Button><Button onClick={signOut}>退出</Button></Stack></Stack></Box>
 <Container maxWidth={false} sx={{py:4}}><Stack gap={2.5}><Stack direction="row" justifyContent="space-between"><Box><Typography variant="h4">{id?'卡片详情':'卡交易流水'}</Typography><Typography color="text.secondary" sx={{mt:1}}>Slash · 真实只读记录</Typography></Box><Button disabled={busy} onClick={()=>setRefresh(n=>n+1)}>刷新已导入数据</Button></Stack>
