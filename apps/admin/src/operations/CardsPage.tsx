...
function CardsContent({channels}:{channels:boolean}){
 ...
 useEffect(()=>{if(channels||!connection)return;let active=true;const timer=setInterval(()=>{if(document.visibilityState!=='visible')return;const q=new URLSearchParams({page:String(page),keyword,cardStatus,revision:connections.find(c=>c.id===connection)?.revision||''});liveGet<Result>(`/admin-api/v1/channel-projections/${connection}/cards?${q}`).then(x=>{if(active){setResult(x);setPollError(false)}}).catch(()=>{if(active)setPollError(true)})},15000);return()=>{active=false;clearInterval(timer)}},[channels,connection,connections,page,keyword,cardStatus]);
