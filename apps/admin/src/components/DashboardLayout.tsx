...
function Navigation({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const livePage = isSlashDemoMode && ["/cards", "/transactions"].includes(location.pathname) && new URLSearchParams(location.search).get("source") !== "demo";
        ...
        <Box sx={{ borderRadius: 2, bgcolor: "grey.100", p: 2 }}>
          <Stack direction="row" alignItems="center" gap={1}>
            ...
            <Typography variant="subtitle2">
              {livePage ? "真实渠道 · 本地只读" : isDemoMode ? "隔离演示环境" : "业务只读 · 管理本地"}
            </Typography>
          </Stack>
          <Typography
            variant="caption"
          ...
          >
            {livePage ? "此页显示已导入的 Slash 真实数据，更新状态见页面；其他演示模块独立。" : isDemoMode
...
export function DashboardLayout() {
  ...
  const livePage=isSlashDemoMode && ["/cards","/transactions"].includes(liveLocation.pathname)&&new URLSearchParams(liveLocation.search).get("source")!=="demo";
  ...
  const initials = useMemo(
    () =>
      String(profile?.nickname || profile?.username || "A")
        .slice(0, 2)
        .toUpperCase(),
    [profile],
  );
        ...
        >
          <Toolbar sx={{ minHeight: { xs: 72, md: 80 }, px: { xs: 2, md: 4 } }}>
            ...
            <TextField
              ...
              placeholder="邮箱、卡号，tx:交易 / order:订单"
              aria-label="全局搜索"
            ...
            <Chip
              ...
              label={livePage ? "Slash 真实数据 / 本地只读" : dataSourceLabel}
              color="success"
              variant="outlined"
