...
function LiveDirectory({kind}:{kind:'cards'|'transactions'}){
 ...
 const query=JSON.stringify({page,pageSize,keyword:params.get('keyword')||'',status:params.get('status')||'',detailedStatus:kind==='transactions'?params.get('detailedStatus')||'':undefined,cardOnly:kind==='transactions'?'true':undefined,group:kind==='cards'?params.get('group')||'':undefined,cardId:kind==='transactions'?params.get('cardId')||'':undefined});
 ...
 return <Stack gap={2}>
  ...
  {kind==='transactions'&&<Alert severity="info">显示最近 30 天内已导入且关联卡片的交易，来源分页及历史更新覆盖可能不完整。金额按来源 status=posted 汇总，不等于账户余额或已确认结算。{data?.summary?.statusReview?` 当前筛选中 ${data.summary.statusReview} 条状态组合待核实。`:''}</Alert>}
  ...
  </Stack>{loading&&<LinearProgress/>}<DataGrid getRowClassName={p=>kind==='transactions'?transactionRowClass(p.row.status,p.row.detailedStatus):''} key={kind} slots={{toolbar:ColumnsToolbar}} initialState={{columns:{columnVisibilityModel:{customer:kind==='cards',id:false,groups:false,observedAt:false,date:false,fetchError:false}}}} sx={{...transactionRowStyles,"& .MuiDataGrid-cell":{fontVariantNumeric:"tabular-nums"},"& .MuiDataGrid-row":{cursor:"pointer"}}} autoHeight rows={data?.rows||[]} columns={columns} rowCount={data?.total||0} paginationMode="server" paginationModel={{page,pageSize}} pageSizeOptions={[20]} onPaginationModelChange={p=>{const q=new URLSearchParams(params);q.set('page',String(p.page));setParams(q);}} disableRowSelectionOnClick disableColumnSorting onRowClick={p=>void show(p.row.id)} localeText={zhCN.components.MuiDataGrid.defaultProps.localeText}/></Paper>
