...
func (s *Server) cryptoSourceAPI(w http.ResponseWriter, r *http.Request) {
	...
	if connection != "" {
		var allowed bool
		e = tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM effective_crypto_connection_grants WHERE namespace=$1 AND connection_id=$2 AND user_id=$3 AND permission=$4)`, svc.NS(), connection, p.ID, permission).Scan(&allowed)
		if e != nil {
			cryptoFail(w, e)
			return
		}
	...
	if connection == "" {
		...
		rows, err := tx.Query(ctx, `SELECT g.connection_id,COALESCE(s.state,'never_synced'),s.last_success,COALESCE(s.error,''),COALESCE(s.page,1),COALESCE(s.generation,0) FROM effective_crypto_connection_grants g LEFT JOIN crypto_sync s ON s.namespace=g.namespace AND s.connection_id=g.connection_id WHERE g.namespace=$1 AND g.user_id=$2 AND g.permission='read' ORDER BY g.connection_id LIMIT 101`, svc.NS(), p.ID)
		if err != nil {
			cryptoFail(w, err)
			return
		}
