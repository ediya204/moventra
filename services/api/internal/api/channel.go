...
func (s *Server) channelRead(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasSuffix(r.URL.Path, "/cvv/reveal") || strings.HasSuffix(r.URL.Path, "/details/reveal") {
	...
	if client && kind == "card" && id != "" && r.Method == "GET" && len(data) == 1 {
		...
		_, detailsSupported := s.CardSecrets.(cardDetailsProvider)
		row["detailsAvailable"] = row["cvvAvailable"] == true && detailsSupported
		row["fundingCardId"] = nil
		if svc, err := s.fundsReadService(); err == nil && (svc.Ledger.IsLive() || s.Issuing != nil && s.Issuing.Mode == "isolated" && s.Issuing.Funds == svc.Ledger) {
			var funding string
		...
		if s.Issuing != nil && s.Issuing.Funds != nil {
			var order string
			err := tx.QueryRow(r.Context(), `SELECT order_id::text FROM issuing_card_projections WHERE customer_id=$1 AND connection_id=$2 AND external_card_id=$3`, customer, connection, id).Scan(&order)
			if err == nil {
				row["issuingOrderId"] = order
			} else if err != pgx.ErrNoRows {
				fail(w, 503, "temporarily_unavailable")
				return
			}
