...
func TestRoleMigration(t *testing.T) {
 ...
 `); err != nil {
	...
	if err = pool.QueryRow(ctx, `SELECT (SELECT count(*) FROM channel_read_grants),(SELECT count(*) FROM onboarding_events),(SELECT count(*) FROM schema_migrations)`).Scan(&channels, &approvals, &versions); err != nil || channels != 1 || approvals != 1 || versions != 9 {
