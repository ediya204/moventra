...
async function transport(admin){
 ...
 const code=transpile('../../packages/shared/src/auth/liveApi.ts').replace(/from ["']([^"']+)["']/g,(_,specifier)=>`from ${JSON.stringify(specifier==='./fundsContract'?uri(transpile('../../packages/shared/src/auth/fundsContract.ts')):specifier==='./cardSnapshotContract'?uri(transpile('../../packages/shared/src/auth/cardSnapshotContract.ts')):specifier==='./ledgerContract'?uri(transpile('../../packages/shared/src/auth/ledgerContract.ts')):mock)}`);
