...
test('checkout requires both declarations, resets on amount change, and restores identical payment after timeout/remount',async()=>{
 ...
 assert.match(text(tree),/资金中心 USD 可用余额/);assert.match(text(tree),/从资金中心 USD 钱包扣除/);assert.ok(buttons(tree).some(b=>b.props.to==='/portal/funds/exchange'));
