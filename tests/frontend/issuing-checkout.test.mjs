...
test('real pilot displays its cap without submitting an order automatically',async()=>{
 storage.clear();fixture.calls=[];fixture.wallet={availableMinor:'100009',currency:'USD',mode:'pilot',fundingSource:'funds_wallet',executionEnabled:true,pilot:{bin:'43612080',maxCards:1,fundingMinor:'2000',feeCapMinor:'1000',totalCapMinor:'3000',expiresAt:'2026-09-20T00:00:00Z'}};
 fixture.products=[{id,name:'43612080',bin:'43612080',network:'visa',minimumMinor:'2000',feeMinor:'1000',blockedReason:''}];
 let tree;
 try {tree=await mount();await autoQuote();assert.match(text(tree),/真实开卡验收/);assert.match(text(tree),/30.00/);assert.equal(tree.root.findAllByType('input').find(n=>n.props.label==='初始卡余额 · USD').props.disabled,true);assert.equal(fixture.calls.some(c=>c.path.endsWith('/orders')&&c.body),false);assert.equal(buttons(tree).find(b=>String(b.props.children).startsWith('确认开卡')).props.disabled,true)} finally {if(tree)await act(()=>tree.unmount());delete fixture.wallet}
});
