...
const config=JSON.parse(readFileSync(new URL('../../packages/shared/src/config/firebase.web.json',import.meta.url)));
...
const db='moventra_test_live_'+Date.now();let databaseCreated=false;
try {
 ...
 const api=resolve(new URL('../../services/api',import.meta.url).pathname);
