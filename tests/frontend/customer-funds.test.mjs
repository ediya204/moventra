...
const walletModule=uri(compile('../../apps/client/src/portal/ProductionWallet.tsx').replace(/from ["']([^"']+)["']/g,(_,n)=>'from '+JSON.stringify(n==='@mui/material'?shell:n.endsWith('/cryptoContract')?contract:n.endsWith('/cryptoApi')?api:resolve(n))));const Wallet=(await import(walletModule)).default;
