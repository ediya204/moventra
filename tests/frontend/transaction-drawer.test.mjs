...
const chipSource=readFileSync(new URL('../../apps/admin/src/components/TransactionStatusChip.tsx',import.meta.url),'utf8');
const chip=uri(ts.transpileModule(chipSource,{compilerOptions:{target:ts.ScriptTarget.ES2022,module:ts.ModuleKind.ESNext,jsx:ts.JsxEmit.ReactJSX}}).outputText.replace(/from ["']([^"']+)["']/g,(_,name)=>'from '+JSON.stringify(name.startsWith('react')?pathToFileURL(require.resolve(name)).href:name.includes('cardTransactionFields')?new URL('../../apps/admin/src/components/cardTransactionFields.ts',import.meta.url).href:name.includes('transactionVisuals')?uri('export const transactionChipSx=()=>({});'):mock)));
outputText=outputText.replace(/from ["']([^"']+)["']/g,(_,name)=>`from ${JSON.stringify(name.includes('TransactionStatusChip')?chip:name.startsWith('react')?pathToFileURL(require.resolve(name)).href:name.includes('cardTransactionFields')?new URL('../../apps/admin/src/components/cardTransactionFields.ts',import.meta.url).href:name.includes('channelOwnership')?new URL('../../apps/admin/src/components/channelOwnership.ts',import.meta.url).href:mock)}`);
...
test('drawer keeps posted date absent for pending and does not invent customer ownership',()=>{
 ...
 assert.match(html,/−3.04/);assert.match(html,/尚未入账/);assert.match(html,/归属未查询/);assert.match(html,/Cardholder Name/);assert.match(html,/备注未采集/);assert.match(html,/未提供或旧记录未采集/);
});
...
test('authoritative customer assignment overrides legacy drawer ownership',()=>{
 const transaction={...base.transaction,internal:{customerName:'Legacy name'}};
 assert.match(render({transaction:{...transaction,customerAssignment:{state:'assigned',id:'c1',name:'Customer A'}}}),/所属账户.*Customer A/);
 const restricted=render({transaction:{...transaction,customerAssignment:{state:'restricted'}}});
 assert.match(restricted,/已分配 · 无查看权限/);assert.doesNotMatch(restricted,/Legacy name/);
 assert.match(render({transaction:{...transaction,customerAssignment:{state:'unassigned'}}}),/未分配/);
});
