...
const compiled=source('../../apps/client/src/portal/ClientHome.tsx').replace(/from ["']([^"']+)["']/g,(_,name)=>'from '+JSON.stringify(name.endsWith('/SectionNavigation')?sectionNav:name.endsWith('/FundsNavigation')?fundsNav:name==='@mui/material'?shell:name==='@iconify/react'?icon:name.endsWith('/AuthContext')?auth:name.endsWith('/liveApi')?api:name.endsWith('/SessionPage')?session:name.endsWith('/BrandLogo')?brand:(name==='../issuing/CardIssuing'||name==='./CardOverview'||name==='./CardSnapshots'||name==='./TestWallet'||name==='./ProductionWallet'||name.endsWith('/finance/OnlineFunds')||name.endsWith('/finance/CryptoFunds')||name.endsWith('/finance/CustomerFunds')||name.endsWith('/finance/ManualFunds'))?uri('export default ()=>null;'):name==='./workspaceNavigation'?navigation:name.endsWith('/auth/onboarding')?admission:name.endsWith('/onboarding/OnboardingPanel')?panel:resolve(name)));
...
const panelCode=source('../../packages/shared/src/onboarding/OnboardingPanel.tsx').replace(/from ["']([^"']+)["']/g,(_,name)=>'from '+JSON.stringify(name.endsWith('/SectionNavigation')?sectionNav:name.endsWith('/FundsNavigation')?fundsNav:name==='@mui/material'?shell:name.endsWith('/liveApi')?api:name.endsWith('/onboarding')?admission:resolve(name)));
