...
test('own pending order cannot be approved; payment confirmation is separate from approval',async()=>{
 state.reads=[];memory.clear();let t=await mount({orderId});const order={id:orderId,customerId:id,actorId:'staff',direction:'debit',source:'offline_payout',currency:'USD',amountMinor:'100',state:'pending_review',revision:1,walletBeforeMinor:null,walletAfterMinor:null,createdAt:new Date().toISOString()};await act(async()=>{state.reads[0].resolve({order,events:[],mode:'shadow',enabled:true});await flush()});assert.equal(button(t,'批准'),undefined);assert.ok(button(t,'取消申请'));await act(()=>t.unmount());
 state.reads=[];t=await mount({orderId});await act(async()=>{state.reads[0].resolve({order:{...order,actorId:'other',state:'awaiting_payment'},events:[],mode:'shadow',enabled:true});await flush()});assert.ok(button(t,'确认线下已付款'));assert.equal(button(t,'批准'),undefined);await act(()=>t.unmount());
});
