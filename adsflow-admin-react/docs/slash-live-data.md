...
真实数据只位于后端 `adsflow-api/.slash-preview/live.sqlite`；真实 key 位于既有 `.env.slash.local`，不进入前端或返回给浏览器。数据文件和配置受 Git 忽略与私有文件权限保护。字段白名单不含 PAN/CVV/OTP。

## 初始化与运行

在 `adsflow-api` 中用已批准的导出批次初始化一次：
