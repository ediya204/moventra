...
导航由 `src/admin/navigation.ts` 集中定义。按最长路径选择当前菜单，避免卡片详情与卡资产页面同时高亮；通过首页进入下级页面时自动展开所属分组。原 `/workbench` 地址仍有效，现在展示管理总览。全局搜索支持邮箱、`user:`、`order:`、`tx:` 和卡片关键词。
...
- 完全隔离演示：`http://127.0.0.1:8852/workbench`，登录 `demo@adsflow.local` / `demo-only`；必要时点击“启用本地管理演示”。
...
4. 浏览器保存总后台名称“ADSFLOW 管理总后台”，操作日志出现 `settings.update`。
