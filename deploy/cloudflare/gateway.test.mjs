...
test('production display blocks legacy test APIs and redirects old pages',async()=>{for(const kind of ['client','admin']){for(const path of [`/${kind}-api/v1/customers/10000000-0000-0000-0000-000000000001/test-funds`,`/${kind}-api/v1/customers/10000000-0000-0000-0000-000000000001/test-wallet`]){const r=await handle(new Request('https://web.invalid'+path),{...env,SITE_KIND:kind,FUNDS_DISPLAY_MODE:'production'},()=>assert.fail('legacy request forwarded'));assert.equal(r.status,404)}}const r=await handle(new Request('https://web.invalid/portal/test-funds/history'),{...env,SITE_KIND:'client',FUNDS_DISPLAY_MODE:'production'});assert.equal(r.status,302);assert.equal(r.headers.get('Location'),'/portal/funds')});
